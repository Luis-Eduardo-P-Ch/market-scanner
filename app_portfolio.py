...
from peg_scanner_model import PEGScanner
...
# TABS PRINCIPALES
# ─────────────────────────────────────────────
tab_portfolio, tab_scanner, tab_dividendos, tab_peg = st.tabs(["📈 Portafolio", "📉 Scanner de Mercados", "💰 Ranking de Dividendos", "📊 PEG Ratio"])
...
# ══════════════════════════════════════════════
# TAB 4 — PEG RATIO SCANNER
# ══════════════════════════════════════════════
with tab_peg:
    st.markdown("### 📊 Scanner de PEG Ratio")
    st.markdown(
        "El **PEG Ratio** (Price/Earnings to Growth) combina el P/E con la tasa de crecimiento esperada. "
        "Un PEG **menor a 1** puede indicar que la acción está subvalorada respecto a su potencial de crecimiento."
    )
    st.info("💡 Regla general: PEG < 1 → posible oportunidad · PEG > 1 → posible sobrevaluación · Solo NYSE/NASDAQ.")
    st.markdown("---")

    peg_ctrl1, peg_ctrl2 = st.columns([2, 1])

    with peg_ctrl1:
        peg_vista = st.selectbox(
            "Vista:",
            options=["Tablas", "Gráfico de barras"],
            key="peg_vista",
        )
    ...
    with peg_ctrl2:
        st.markdown("<br>", unsafe_allow_html=True)
        peg_btn = st.button("📊  ESCANEAR PEG RATIO", key="btn_peg")

    if peg_btn:
        with st.spinner("Descargando PEG Ratios desde Yahoo Finance... (puede tardar ~2 min)"):
            try:
                scanner_peg = PEGScanner()
                prog_peg = st.progress(0, text="Analizando PEG Ratios...")

                def _prog_peg(val):
                    prog_peg.progress(val, text=f"Analizando PEG Ratios... {int(val*100)}%")

                res_peg = scanner_peg.escanear(progress_callback=_prog_peg)
                prog_peg.empty()

                st.session_state.peg_resultados = res_peg
                st.session_state.peg_vista_val  = peg_vista

            except Exception as e:
                st.error(f"Error al escanear PEG Ratios: {e}")

    if "peg_resultados" in st.session_state:
        res_peg  = st.session_state.peg_resultados
        vista_peg = st.session_state.get("peg_vista_val", "Tablas")

        df_bajo  = res_peg.get("bajo")
        df_alto  = res_peg.get("alto")
        df_todos = res_peg.get("todos")

        if df_bajo is None or df_bajo.empty:
            st.warning("No se encontraron datos de PEG Ratio. Intentá nuevamente.")
        else:
            if vista_peg == "Tablas":
                ...
                with col_bajo:
                    st.markdown(
                        '<div class="scanner-header" style="color:#1a7a4a;">🟢 PEG Ratio más bajo (posible valor)</div>',
                        unsafe_allow_html=True,
                    )
                    rows_html = ""
                    for i, row in df_bajo.iterrows():
                        ticker  = str(row["Ticker"])
                        nombre  = str(row.get("Nombre", ticker))[:26]
                        peg_val = row["PEG Ratio"]
                        sector  = str(row.get("Sector", ""))[:20]
                        color_class = "rank-change-gainer" if peg_val < 1 else "neutral"
                        rows_html += f"""
                        <div class="rank-row">
                            <span class="rank-num">{i+1}</span>
                            <span class="rank-ticker">{ticker}</span>
                            <span style="font-size:0.72rem; color:#aaa; flex:2; padding:0 0.4rem;">{sector}</span>
                            <span class="{color_class}" style="font-weight:700;">{peg_val:.2f}</span>
                        </div>"""
                    st.markdown(f'<div class="scanner-card">{rows_html}</div>', unsafe_allow_html=True)
                ...
                with col_alto:
                    st.markdown(
                        '<div class="scanner-header" style="color:#c0392b;">🔴 PEG Ratio más alto (posible sobrevaluación)</div>',
                        unsafe_allow_html=True,
                    )
                    rows_html = ""
                    for i, row in df_alto.iterrows():
                        ticker  = str(row["Ticker"])
                        nombre  = str(row.get("Nombre", ticker))[:26]
                        peg_val = row["PEG Ratio"]
                        sector  = str(row.get("Sector", ""))[:20]
                        ...
                        <div class="rank-row">
                            ...
                            <span class="rank-ticker">{ticker}</span>
                            <span style="font-size:0.72rem; color:#aaa; flex:2; padding:0 0.4rem;">{sector}</span>
                            <span class="rank-change-loser" style="font-weight:700;">{peg_val:.2f}</span>
                        </div>"""
                    st.markdown(f'<div class="scanner-card">{rows_html}</div>', unsafe_allow_html=True)
                ...
                with col_g1:
                    if df_bajo is not None and not df_bajo.empty:
                        colors_b = [f"rgba(26,122,74,{0.35 + 0.065*i})" for i in range(len(df_bajo))]
                        fig_bajo = go.Figure(go.Bar(
                            x=df_bajo["PEG Ratio"],
                            y=df_bajo["Ticker"],
                            orientation="h",
                            marker_color=colors_b,
                            text=df_bajo["PEG Ratio"].apply(lambda v: f"{v:.2f}"),
                            textposition="outside",
                        ))
                        fig_bajo.update_layout(
                            title="🟢 Menores PEG Ratios",
                            xaxis_title="PEG Ratio",
                            yaxis=dict(autorange="reversed"),
                            height=420,
                            template="plotly_white",
                            margin=dict(l=10, r=70, t=40, b=30),
                        )
                        fig_bajo.add_vline(x=1, line_dash="dash", line_color="#aaa",
                                           annotation_text="PEG = 1", annotation_position="top right")
                        st.plotly_chart(fig_bajo, width="stretch")

                with col_g2:
                    if df_alto is not None and not df_alto.empty:
                        colors_a = [f"rgba(192,57,43,{0.35 + 0.065*i})" for i in range(len(df_alto))]
                        fig_alto = go.Figure(go.Bar(
                            x=df_alto["PEG Ratio"],
                            y=df_alto["Ticker"],
                            orientation="h",
                            marker_color=colors_a,
                            text=df_alto["PEG Ratio"].apply(lambda v: f"{v:.2f}"),
                            textposition="outside",
                        ))
                        fig_alto.update_layout(
                            title="🔴 Mayores PEG Ratios",
                            xaxis_title="PEG Ratio",
                            yaxis=dict(autorange="reversed"),
                            height=420,
                            template="plotly_white",
                        ...
                        fig_alto.add_vline(x=1, line_dash="dash", line_color="#aaa",
                                           annotation_text="PEG = 1", annotation_position="top right")
                        st.plotly_chart(fig_alto, width="stretch")

            # ── Tabla completa descargable ──────────────────────────
            with st.expander("📋 Ver tabla completa de PEG Ratios"):
                if df_todos is not None and not df_todos.empty:
                    st.dataframe(
                        df_todos.style.format({
                            "PEG Ratio": "{:.2f}",
                            "P/E Ratio": lambda v: f"{v:.2f}" if v else "N/D",
                            "Precio":    lambda v: f"${v:.2f}" if v else "N/D",
                        }),
                        width="stretch",
                        hide_index=True,
                    )
            ...
            st.markdown(
                '<p class="warning-text">* PEG Ratio = P/E ÷ Tasa de crecimiento esperada. '
                'Datos de Yahoo Finance. Un PEG bajo no garantiza rentabilidad. Solo con fines educativos.</p>',
                unsafe_allow_html=True,
            )
        ...
        <div style="text-align:center; padding: 3rem 2rem; color:#bbb;">
            <p style="font-size:4rem; margin:0;">📊</p>
            <p style="font-family:'Syne',sans-serif; font-size:1.1rem; color:#aaa; margin-top:1rem;">
                Hacé clic en <strong style="color:#1a1a1a;">ESCANEAR PEG RATIO</strong><br>
                para ver el ranking de acciones NYSE/NASDAQ
            </p>
        </div>
        """, unsafe_allow_html=True)

...
from per_scanner_model import PERScanner
...
# TABS PRINCIPALES
# ─────────────────────────────────────────────
tab_portfolio, tab_scanner, tab_dividendos, tab_peg = st.tabs(["📈 Portafolio", "📉 Scanner de Mercados", "💰 Ranking de Dividendos", "📊 Ranking P/E Ratio"])
...
# ══════════════════════════════════════════════
# TAB 4 — P/E RATIO SCANNER
# ══════════════════════════════════════════════
with tab_peg:
    st.markdown("### 📊 Ranking de P/E Ratio (PER)")
    st.markdown(
        "El **P/E Ratio** (Precio / Beneficio) indica cuántas veces el mercado paga el beneficio anual de una empresa. "
        "Un PER **bajo** puede señalar una acción subvalorada; un PER **alto** refleja expectativas elevadas de crecimiento."
    )
    st.markdown("---")

    # ── Controles ──────────────────────────────────────────────────
    per_c1, per_c2, per_c3, per_c4 = st.columns([2, 1, 1, 1])

    with per_c1:
        per_mercado = st.radio(
            "Mercado:",
            options=["NYSE / NASDAQ", "Mercado Argentino (ByMA)"],
            horizontal=True,
            key="per_mercado",
    ...
    with per_c2:
        per_tipo = st.radio(
            "Tipo de P/E:",
            options=["trailing", "forward", "ambos"],
            format_func=lambda x: {
                "trailing": "Trailing (real)",
                "forward":  "Forward (estimado)",
                "ambos":    "Ambos",
            }[x],
            key="per_tipo",
        )
    ...
    with per_c4:
        st.markdown("<br>", unsafe_allow_html=True)
        per_btn = st.button("📊  ESCANEAR P/E RATIO", key="btn_per")

    # ── Info contextual por tipo ────────────────────────────────────
    if per_tipo == "trailing":
        st.info("💡 **Trailing P/E**: basado en beneficios reales de los últimos 12 meses. Más estable y confiable.")
    elif per_tipo == "forward":
        st.info("💡 **Forward P/E**: basado en estimaciones de analistas para los próximos 12 meses. Más especulativo.")
    else:
        st.info("💡 Mostrando **Trailing y Forward P/E** en columnas separadas. El ranking principal usa Trailing P/E.")

    st.markdown("<br>", unsafe_allow_html=True)

    if per_btn:
        mercado_key = "byma" if "ByMA" in per_mercado else "nyse"
        lbl_mercado = per_mercado
        with st.spinner(f"Descargando P/E Ratios de {lbl_mercado}... (puede tardar ~1–2 min)"):
            try:
                scanner_per = PERScanner(mercado=mercado_key)
                prog_per = st.progress(0, text="Analizando P/E Ratios...")

                def _prog_per(val):
                    prog_per.progress(val, text=f"Analizando P/E Ratios... {int(val*100)}%")

                res_per = scanner_per.escanear(tipo_pe=per_tipo, progress_callback=_prog_per)
                prog_per.empty()

                st.session_state.per_resultados  = res_per
                st.session_state.per_vista_val   = per_vista
                st.session_state.per_mercado_lbl = per_mercado
                st.session_state.per_tipo_val    = per_tipo

            except Exception as e:
                st.error(f"Error al escanear P/E Ratios: {e}")

    # ── Mostrar resultados ──────────────────────────────────────────
    if "per_resultados" in st.session_state:
        res_per    = st.session_state.per_resultados
        vista_per  = st.session_state.get("per_vista_val", "Tablas")
        tipo_per   = st.session_state.get("per_tipo_val", "trailing")
        lbl_per    = st.session_state.get("per_mercado_lbl", "")

        df_bajo  = res_per.get("bajo")
        df_alto  = res_per.get("alto")
        df_todos = res_per.get("todos")
        col_pe   = res_per.get("col_pe", "Trailing P/E")

        if df_bajo is None or df_bajo.empty:
            st.warning("No se encontraron datos de P/E para este mercado. Intentá nuevamente.")
        else:
            es_byma = ".BA" in str(df_bajo["Ticker"].iloc[0]) if not df_bajo.empty else False

            def _fmt_ticker(t):
                return str(t).replace(".BA", "")

            def _render_per_tabla(df, titulo, color_class, col_val):
                st.markdown(
                    f'<div class="scanner-header" style="color:{color_class};">{titulo}</div>',
                    unsafe_allow_html=True,
                )
                rows_html = ""
                for i, row in df.iterrows():
                    ticker  = _fmt_ticker(row["Ticker"])
                    sector  = str(row.get("Sector", "N/D"))[:22]
                    pe_val  = row.get(col_val)
                    pe_str  = f"{pe_val:.1f}x" if pe_val else "—"

                    # Segunda columna de PE si modo ambos
                    extra = ""
                    if tipo_per == "ambos":
                        other_col = "Forward P/E" if col_val == "Trailing P/E" else "Trailing P/E"
                        other_val = row.get(other_col)
                        other_lbl = "fwd" if other_col == "Forward P/E" else "trail"
                        extra = f'<span style="font-size:0.72rem; color:#aaa; margin-right:0.5rem;">{other_lbl}: {f"{other_val:.1f}x" if other_val else "—"}</span>'

                    rows_html += f"""
                    <div class="rank-row">
                        <span class="rank-num">{i+1}</span>
                        <span class="rank-ticker">{ticker}</span>
                        <span style="font-size:0.72rem; color:#aaa; flex:2; padding:0 0.4rem;">{sector}</span>
                        {extra}
                        <span style="font-family:'Syne',sans-serif; font-weight:700;">{pe_str}</span>
                    </div>"""
                st.markdown(f'<div class="scanner-card">{rows_html}</div>', unsafe_allow_html=True)

            if vista_per == "Tablas":
                col_bajo, col_alto = st.columns(2)
                with col_bajo:
                    _render_per_tabla(df_bajo, "🟢 P/E más bajo — posible valor", "#1a7a4a", col_pe)
                with col_alto:
                    _render_per_tabla(df_alto, "🔴 P/E más alto — posible sobrevaluación", "#c0392b", col_pe)

            else:
                col_g1, col_g2 = st.columns(2)

                def _make_bar(df, titulo, is_low):
                    vals = df[col_pe].fillna(0)
                    base_color = (26, 122, 74) if is_low else (192, 57, 43)
                    colors = [f"rgba({base_color[0]},{base_color[1]},{base_color[2]},{0.35 + 0.065*i})" for i in range(len(df))]
                    fig = go.Figure(go.Bar(
                        x=vals,
                        y=df["Ticker"].apply(_fmt_ticker),
                        orientation="h",
                        marker_color=colors,
                        text=vals.apply(lambda v: f"{v:.1f}x"),
                        textposition="outside",
                    ))
                    fig.update_layout(
                        title=titulo,
                ...
                with col_g1:
                    st.plotly_chart(_make_bar(df_bajo, f"🟢 Menor {col_pe}", True),  width="stretch")
                with col_g2:
                    st.plotly_chart(_make_bar(df_alto, f"🔴 Mayor {col_pe}", False), width="stretch")

            # ── Tabla completa descargable ──────────────────────────
            with st.expander("📋 Ver tabla completa de P/E Ratios"):
                if df_todos is not None and not df_todos.empty:
                    df_show = df_todos.copy()
                    df_show["Ticker"] = df_show["Ticker"].apply(_fmt_ticker)
                    fmt = {"Trailing P/E": "{:.2f}", "Forward P/E": "{:.2f}"}
                    if "Precio" in df_show.columns:
                        fmt["Precio"] = "${:.2f}"
                    st.dataframe(
                        df_show.style.format(fmt, na_rep="N/D"),
                        width="stretch",
                        hide_index=True,
                    )

            st.markdown(
                '<p class="warning-text">* Trailing P/E = Precio / EPS últimos 12 meses. '
        ...
        <div style="text-align:center; padding: 3rem 2rem; color:#bbb;">
            <p style="font-size:4rem; margin:0;">📊</p>
            <p style="font-family:'Syne',sans-serif; font-size:1.1rem; color:#aaa; margin-top:1rem;">
                Seleccioná el mercado y el tipo de P/E,<br>
                luego hacé clic en <strong style="color:#1a1a1a;">ESCANEAR P/E RATIO</strong>
            </p>
        </div>
        """, unsafe_allow_html=True)
